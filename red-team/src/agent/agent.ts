...
import type { PASRIntegration } from './pasr.js';
...
export interface AgentOptions {
  ...
  engagement?: string;
  /** PASR integration — optional subsystem wiring for policy, evidence,
   *  experience, and coverage tracking. When present, the agent records
   *  experiences after tool calls and injects PASR context into prompts. */
  pasr?: PASRIntegration;
}
...
export class Agent {
  ...
  readonly pasr: PASRIntegration | undefined;
  ...
  async run(
    ...
    // Wire the PASR event sink so policy/evidence/experience events
    // are forwarded to the UI during this run.
    this.pasr?.setEventSink(emit);
    this.running = true;
    try {
  ...
  private async runInner(
    ...
    // PASR context: exploit coverage patterns and successful strategies.
    const pasrContext = await this.buildPasrContext();
    if (pasrContext && last) {
      working.splice(working.length - 1, 0, { role: 'system', content: pasrContext });
    }
  ...
  private recordToolResult(
    ...
    // PASR: record experience for this tool call (fire-and-forget).
    if (this.pasr?.active) {
      void this.pasr.recordExperience({
        category: toolCategory(tc.function.name),
        action: tc.function.name,
        target: this.target.baseURL() || this.target.name() || 'unknown',
        outcome: res.errStr ? 'failure' : 'success',
        duration: res.durationMs,
        notes: res.errStr || undefined,
      });
      // Emit experience-recorded event for UI observability.
      emit({
        type: 'experience-recorded',
        experienceId: `${tc.function.name}-${Date.now()}`,
        category: toolCategory(tc.function.name),
        outcome: res.errStr ? 'failure' : 'success',
      });
    }
  ...
  private async buildPasrContext(): Promise<string> {
    if (!this.pasr?.active) return '';
    try {
      return await this.pasr.buildPromptContext();
    } catch {
      return '';
    }
  }
...
const TOOL_CATEGORY_MAP: Record<string, import('../experience/types.js').ExperienceCategory> = {
  http: 'recon',
  shell: 'recon',
  BashTool: 'recon',
  curl: 'recon',
  ...
  coverage: 'report',
  finding: 'report',
  confirm_finding: 'report',
  load_skill: 'recon',
  ask: 'recon',
  web_fetch: 'recon',
  web_search: 'recon',
  browser_navigate: 'recon',
  browser_capture_screenshot: 'recon',
  browser_click: 'exploit',
  browser_type: 'exploit',
  browser_select: 'exploit',
  browser_hover: 'exploit',
  browser_press_key: 'exploit',
  browser_drag: 'exploit',
  browser_scroll: 'exploit',
  read_payloads: 'recon',
  read_skill_file: 'recon',
};

function toolCategory(
  toolName: string,
): import('../experience/types.js').ExperienceCategory {
  return TOOL_CATEGORY_MAP[toolName] ?? 'recon';
}
