...
describe('session.Store', () => {
  ...
  it.skipIf(process.platform === 'win32')('saved file has 0o600 perms', async () => {
    const store = Store.newWithID(tmp, newID());
    await store.save([{ role: 'user', content: 'x' }], null);
  ...
  it.skipIf(process.platform === 'win32')('writes context snapshots under the pentesterflow context directory', async () => {
