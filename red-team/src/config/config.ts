...
export async function save(cfg: Config): Promise<void> {
  ...
  try {
    ...
    await fh.close();
    fh = undefined;
    await rename(tmp, path).catch((err: NodeJS.ErrnoException) => {
      // Windows cannot rename over an existing file (EPERM). Remove the
      // destination first, then rename — this breaks atomicity slightly but
      // is safe here because the .tmp write is already fsynced.
      if (err.code === 'EPERM' && process.platform === 'win32') {
        return unlink(path).then(() => rename(tmp, path));
      }
      throw err;
    });
