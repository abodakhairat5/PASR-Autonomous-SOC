...
function applyAgentEvent(state: AppState, ev: AgentEvent): AppState {
  switch (ev.type) {
    ...
    case 'policy-decision':
      if (ev.action === 'deny' || ev.action === 'require-approval') {
        return {
          ...state,
          transcript: [
            ...state.transcript,
            { kind: 'system', text: `policy: ${ev.action} ${ev.hostname} — ${ev.reason}`, color: ev.action === 'deny' ? 'red' : 'yellow' },
          ],
        };
      }
      return state;
    case 'evidence-captured':
      return {
        ...
        transcript: [
          ...state.transcript,
          { kind: 'system', text: `evidence: captured ${ev.evidenceId} (${ev.tool})` },
        ],
      };
