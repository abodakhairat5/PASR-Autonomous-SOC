...
async function main(): Promise<number> {
  ...
  // Enforcement subsystems — always created regardless of --pasr. These provide
  // scope enforcement, rate limiting, and evidence capture by default.
  const policyEngine = new PolicyEngine(target);
  const rateLimiter = new RateLimiter(target.scope().rateLimit);
  const evidenceStore = new EvidenceStore(`findings/evidence-${sessionID}`);

  // Observability subsystems — only created when --pasr is set. These add
  // experience tracking, exploit coverage, and audit logging on top of the
  // always-on enforcement layer.
  const experienceStore = flags.pasr
    ? new ExperienceStore(`findings/experience-${sessionID}.json`)
    : undefined;
  const extendedCoverage = flags.pasr
    ? new ExtendedCoverageStore(`findings/exploit-coverage-${sessionID}.json`)
    : undefined;
  const auditLogger = flags.pasr ? new AuditLogger(`findings/audit-${sessionID}`) : undefined;
